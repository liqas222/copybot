...
PAPER = {"cash": PAPER_START, "pos": {}, "hist": []}   # pos: key(tuple) -> dict; hist: [[t_ms, equity], ...]
...
def open_copy(ex, w, equity):
    ...
    try:
        ...
        tg("✅ KOPIERT %s %s · Größe %.4f @ ~$%.4f · %dx %s · TP +%.0f%% ROE @ $%s"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/pause":
            ...
            save_secrets()
            if TG["token"]:
                threading.Thread(target=lambda: tg("✅ Telegram verbunden — ab jetzt bekommst du hier Benachrichtigungen."), daemon=True).start()
            ...
            save_secrets()
            tg("⚙️ Take-Profit gesetzt · Crypto +%.0f%% · Stocks +%.0f%% (gilt für neue Trades)"
...
def run_paper():
    ...
    if loaded:
        ...
        tg("💾 Vorheriger Papier-Stand geladen · Kasse $%.2f · %d offene Position(en)"
    ...
    while True:
        try:
            ...
            if not announced:
                ...
                tg("👀 Beobachte ab jetzt — bestehende Whale-Positionen werden NICHT kopiert. Nur NEUE, je %d× bestätigt (Open & Close)." % CLOSE_CONFIRM)
            ...
            with LOCK:
                ...
                for key in list(PAPER["pos"].keys()):
                    ...
                    if pnl <= -p["margin"]:
                        ...
                        events.append("🎯 TP +%.0f%% ROE %s — Gewinn $%.2f" % (tp * 100, p["bare"], pnl))
            ...
            for key in cand:
                ...
                if STATE["paused"]:
                    tg("⏭️ %s übersprungen — pausiert." % w["bare"]); continue
                if killed:
                    tg("⏭️ %s übersprungen — Kill-Switch." % w["bare"]); continue
                msg = None
                with LOCK:
                    if len(PAPER["pos"]) >= MAX_POSITIONS:
                        msg = "⏭️ %s übersprungen — 5 Slots belegt." % w["bare"]
                    else:
                        eq = paper_equity(whale, mids)
                        ...
                        msg = ("✅ KOPIERT %s %s · Einsatz $%.2f · %dx %s · TP +%.0f%% ROE"
