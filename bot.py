...
def save_secrets():
    try:
        json.dump({"tg_token": TG["token"], "tg_chat": TG["chat"]}, open(SECRETS_FILE, "w"))
    except Exception as e:
    ...
    while True:
        try:
            ...
            with LOCK:
                ...
                # TP +20% ROE / liquidation (whale still holds it -> stays 'known', no re-open)
                for key in list(PAPER["pos"].keys()):
                    p = PAPER["pos"][key]
                    pnl = _pnl(key, p, whale, mids)
            ...
            for key in cand:
                ...
                with LOCK:
                    if len(PAPER["pos"]) >= MAX_POSITIONS:
                        ...
                        msg = ("✅ KOPIERT %s %s · Einsatz $%.2f · %dx %s · TP +20%% ROE"
